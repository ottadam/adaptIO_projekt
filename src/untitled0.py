...
train_episodes = 5 # tanítási epizódok száma
replay_memory = deque(maxlen = 50_000) # memória mérete
    
# mentés, visszaolvasás elérési útvonala
PATH = "D:/BME/Msc/1. félév\Adaptív rendszerek modellezése/Porjekt/adaptivegame/AI_mentesek_0513_200games" 
...
def train(replay_memory, model, target_model):
    ...
    MIN_REPLAY_SIZE = 500  # csak akkor tanulunk ha van legalább 300 korábbi állapot-akció-jutalom lementve
    if len(replay_memory) < MIN_REPLAY_SIZE:
        #print('replay memory small')
        return

    batch_size = 200  # 150 random elem kiválasztása a memóriából és szétbontása, ezeken tanítunk
...
class RemoteAI:
    ...
    def get_state_and_reward(self, fulljson):
        ...
        if "pos" in jsonData.keys() and "tick" in jsonData.keys() and "active" in jsonData.keys() and "size" in jsonData.keys() and "vision" in jsonData.keys():
                ...
                if field["player"] is not None: 
                    if tuple(field["relative_coord"]) == (0, 0): 
                        if 0 < field["value"] <= 3:  # kajás mezők feldolgozása
                            t = int(field["value"])
                            new_state[5*i+3+t] = 1
                            reward_food = reward_food + field["value"]/25
                        elif field["value"] == 9: # falak feldolgozása
                            new_state[5*i+3+4] = 1
                            reward_food += 0  # jutalom ha kaja van a látómezőben
                            reward_wall += -1/20 # büntetés ha fal van a látómezőben
                        else:           # ürres mező
                            new_state[5*i+3] = 1
                        
                    else: # ha van ellenfél 
                        ...
                        if field["player"]["size"] * 1.1 < jsonData["size"]:
                            reward_hunt = math.sqrt(math.pow((field["relative_coord"][0]),2)+math.pow((field["relative_coord"][1]),2))/2 # jutalom ha elkerüli a nála nagyobb ellenfelet
                            reward_avoid = 0
            ...
            reward = reward_size + reward_food + reward_hunt + reward_avoid # + reward_moved # + reward_alive # összesített jutalom kiszámítása
            #print(reward_moved)
             ...
             if fulljson["payload"]["active"]:
                 ...
                 if e <= self.epsilon:
                     if self.episode > 100:
                         ...
                         self.action = self.NaiveHunterAction(fulljson) # naivehunter bot lép az első 40 játékban a random helyett
                ...
                # választott akció kódolása az elvártg módon
                 ...
                 sendData(json.dumps({"command": "SetAction", "name": "RemotePlayer2", "payload": actstring})) # választott lépés kiküldése
                 ...
                 if Training_ON == True: # ha a tanulás be van kapcsolva
                     ...
                     if len(replay_memory) < 30000:     #az első 5000 lépés eredményét minden esetben letárljuk a memóriába
                         replay_memory.append([self.state, self.action, self.reward, new_state])
                     if len(replay_memory) >= 30000  and self.reward >= self.margin:       # az 5000 lépésen túl már csak a határértéken felül teljesítő lépéseket
                         replay_memory.append([self.state, self.action, self.reward, new_state])
         ...
         if fulljson["type"] == "leaderBoard":
            ...
            # szerzett pontszám kiolvasása az adatcsomagból
             for score in fulljson["payload"]["players"]:
                 ...
                 if score["name"] == 'RemotePlayer2':
                     self.total_score[self.episode]=score['maxSize']
             ...
             self.total_rewards[self.episode]=self.episode_reward # kapott jutalom mentése
             ...
             self.total_avg_reward[self.episode]=avg_reward # mozgóátlagok letárolása
             self.total_avg_score[self.episode]=avg_score   
...
if __name__=="__main__":
    ...
    client.sendData(json.dumps({"command": "SetName", "name": "RemotePlayer2", "payload": None}))
